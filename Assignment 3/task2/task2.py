...
#data.to_csv('task2/train_preprocessed.tsv', sep='\t', index=False)
#testdata.to_csv('task2/test_preprocessed.tsv', sep='\t', index=False)
...
naive_bayes_classifier.fit(X_train_tf, data_hateful)


y_pred = naive_bayes_classifier.predict(X_test_tf)
testdata["pred"] = y_pred
prediction = pd.DataFrame([testdata.id, testdata.pred]).transpose()
prediction.to_csv('/home/yash/Documents/socompML/predictions/T2.csv', sep=',', index=False)